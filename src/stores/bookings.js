...
import { endOfToday, isAfter, isBefore, isToday, subDays, startOfToday } from "date-fns";
...
export default defineStore("bookings", {
  ...
  getters: {
    ...
    dashboardBookings() {
      const firstDate = subDays(startOfToday(), this.last);
    ...
    todaysBookings() {
      return this.bookings.filter(
        (booking) => isToday(booking.startDate) || isToday(booking.endDate)
      );
    },
    todaysActivityBookings() {
      return this.todaysBookings.filter(
        (booking) =>
          (booking.status === "unconfirmed" && isToday(booking.startDate)) ||
          (booking.status === "checked-in" && isToday(booking.endDate))
      );
    },
    createdBookings() {
      ...
      return this.bookings.filter(
        (booking) =>
          isBefore(booking.created_at, lastDate) &&
          isAfter(booking.created_at, firstDate)
      );
    },
  },
});
